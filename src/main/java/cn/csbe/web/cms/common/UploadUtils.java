	...
	public static UploadResult upload(HttpServletRequest request,CommonsMultipartFile mFile,String dirs,Long maxSize){ // 请求参数一定要与form中的参数名对应
		...
		if (!mFile.isEmpty()) {
			...
			try {
				...
				if(strings.length>2){
					String filePath = strings[1] +"/"+strings[2]+ "/" + newFileName;
					result.setValue(filePath);
				}else{
					String filePath = strings[1] +"/"+ newFileName;
					result.setValue(filePath);
				}
				//缩略图
				/*if(dirs.equals("images/wallpager")){
					File fromPic=new File(realPath+"/"+newFileName); 
			        File toPic=new File(realPath+"/"+prefix +"_thum."+ extName);
