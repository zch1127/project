	...
	<table>
		...
		<tr>
			...
			<select id="cateSelect"  class="easyui-combobox"  name="aa" style="width:200px;" data-options="required:true">  
		            ...
		            <option value="1"  <c:if test="${pubNews.type==1 }">selected</c:if>>高僧大德</option>
		            <option value="2"  <c:if test="${pubNews.type==2 }">selected</c:if>>小型活动</option>
		            <option value="3"  <c:if test="${pubNews.type==3 }">selected</c:if>>大型活动</option>
		            <option value="4"  <c:if test="${pubNews.type==4 }">selected</c:if>>历史活动</option>
		            <option value="5"  <c:if test="${pubNews.type==5 }">selected</c:if>>其他</option>
		    </select>  
			</td>
		</tr>
		<tr>
			<td>开始时间</td>
			<td>
			<input class="easyui-datetimebox" id="ksDate" name="publishDate" 
     			style="width:150px">
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><img id="imgPreview" width="150"  src="../images/defaultImg.jpg"/><br/>
			 <input id="picture"  name="picture" type="text" hidden="1" value="${pubNews.picture}" />
