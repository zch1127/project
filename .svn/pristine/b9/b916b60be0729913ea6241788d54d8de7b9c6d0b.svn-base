...
/**
 * window窗口扩展（修改默认onBeforeOpen监听方法）
 * 更正center中height关闭窗口后归零的情况；
 */
var beforeOpenHeight = 0;
jQuery.extend(jQuery.fn.window.defaults,{
	onBeforeOpen:function(){
	beforeOpenHeight = jQuery("body").height();
	jQuery(".panel-tool-close").click(function(){
		if(beforeOpenHeight != 0 ){
			jQuery("body").height(beforeOpenHeight);
		}
	});
	}
});
$.extend($.fn.datagrid.methods,{
	...
	getEditRowsData:function(jq){
		...
		$.each(editRowsIndex,function(i,n){
			_rd.push(jq.datagrid("getEditRow",n));
		});
		return _rd;
	},
	endAllEdits:function(jq){
		...
		$.each(rowIndex,function(i,n){
			jq.datagrid("endEdit",n);
		});
	},
	getUnSelectRows:function(jq){
		//获取未select的行的id数组
		var rows = [];
        var trs=$(jq.datagrid("getPanel")
        	.find("div.datagrid-view2").find("div.datagrid-body tr").not(".datagrid-row-selected"));//datagrid-row 
        $.each(trs,function(){
        	var index = parseInt($(this).attr('datagrid-row-index'));
        	rows.push(jq.datagrid("getData").rows[index]);
        });
...
$.extend($.fn.treegrid.methods,{
	getTreegridEditRowsId:function(jq){
		//获得所有正在编辑中的行序号(rowIndex),返回一个数组
		var _i=[];
        var trs=$(jq.datagrid("getPanel")
        	.find("div.datagrid-view2").find("div.datagrid-body tr").filter(".datagrid-row-editing"));
        $.each(trs,function(){
        	_i.push($(this).attr("node-id"));
        });
        return _i;
	},
	endAllTreegridEdits:function(jq){
		//把所有正在编辑的行都结束编辑
		var rowIndex=jq.treegrid("getTreegridEditRowsId");
		$.each(rowIndex,function(i,n){
			jq.treegrid("endEdit",n);
		});
	},
	getUnSelectRowsId:function(jq){
		//获取未select的行的id数组
		var _i="";
        var trs=$(jq.datagrid("getPanel")
        	.find("div.datagrid-view2").find("div.datagrid-body tr[id^='datagrid-row-r']").not(".datagrid-row-selected"));//datagrid-row 
        $.each(trs,function(){
        	_i += $(this).attr("node-id") + ",";
        });
        _i = _i.substring(0, _i.lastIndexOf(","));
        return _i;
	}
});
