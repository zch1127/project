...
<body>
	<table style="line-height: 35px;">
		...
		<tr>
			...
			<input id="picture"  name="picture" type="text" hidden="1" value="${theme.themeImg}" />
		...
		<td>主题类型：</td>
			...
			<select id="themeSelect"  class="easyui-combobox"  name="aa" style="width:200px;" data-options="required:true">  
		            ...
		            <option value="2"  <c:if test="${theme.type==2}">selected</c:if>>修心</option>
		           ...
		           <!--根据需求删掉善缘   <option value="5"  <c:if test="${theme.type==5}">selected</c:if>>善缘</option>   -->
		            <option value="6"  <c:if test="${theme.type==6}">selected</c:if>>学佛</option>
		    </select>  
			</td>
	    </tr>	
		<tr>
		...
		<tr>
			<td>主题描述</td>
			<td>
				<div style="display: none;"><p id="content" >${theme.htmlUrl }</p></div>
